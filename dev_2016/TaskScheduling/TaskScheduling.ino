...
const unsigned long     TASK_PERIOD_NewTask = 30000000; //3s
...
const unsigned long     TASK_PRESCALAR_NewTask = TASK_PERIOD_NewTask / TIMER_PERIOD;
...
boolean task_NewTask_Ready = false;
...
void loop() {
  ...
  // task_NewTask for development testing
  if (task_NewTask_Ready)
  {
    task_NewTask();
    task_NewTask_Ready = false;
  }
}
...
void waitForTimeSlice()
{
  ...
  static unsigned char newTaskCount = 0;
  ...
  if (newTaskCount >= TASK_PRESCALAR_NewTask)
  {
    task_NewTask_Ready = true;
    newTaskCount = 0;
  }
...
void task_NewTask()
{
  Serial.println("Task 4: New Task"); 
}

...
public:
    ...
    DS2776(uint64_t inputId) : id(inputId) {
        // initialize model
        this->voltage_cell_0        = (int)(NOMINAL_VOLTAGE * 32 / VOLTAGE_PER_INT16);
        this->voltage_cell_1        = (int)(NOMINAL_VOLTAGE * 32 / VOLTAGE_PER_INT16);
        this->temperature           = (int)(NOMINAL_TEMPERATURE * 32 / TEMPERATURE_PER_INT16);
    ...
    DataUpdateStatus updateAllData() {
        ...
        do {
            ...
            count++;
        } while (_readResponse() != DS2776::READ_SUCCESS && count < 3);  // Read the response from the bridge
